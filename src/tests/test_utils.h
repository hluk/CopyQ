...
constexpr auto passwordEntryImportId = "focus:QInputDialog'Password Required'";
...
#define RUN(ARGUMENTS, STDOUT_EXPECTED) \
    do { \
        log("🔵 RUN: " #ARGUMENTS " --- " __FILE__ ":" STR(__LINE__)); \
...
#define KEYS(ARGUMENTS) \
    do { \
        log("🔵 KEYS: " #ARGUMENTS " --- " __FILE__ ":" STR(__LINE__)); \
