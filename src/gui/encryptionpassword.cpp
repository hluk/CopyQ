...
bool removePasswordFromKeychain()
{
    if (!KeychainAccess::isEnabled())
        return false;

    qCInfo(logCategory) << "Removing password from system keychain";
    const bool result = KeychainAccess::deletePassword(
        keychainServiceName(), keychainPasswordKey());

    if (!result)
        return false;

    qCInfo(logCategory) << "Password removed from system keychain";
    return true;
}
...
Encryption::EncryptionKey setUpPassword(const Encryption::SecureArray &password)
{
...
Encryption::SecureArray getStoredPassword()
{
    Encryption::Cleared<QByteArray> ba = qgetenv("COPYQ_PASSWORD");
    if ( !ba.isEmpty() ) {
        qCInfo(logCategory) << "Encryption password loaded from env variable COPYQ_PASSWORD";
    } else {
        ba = loadPasswordFromKeychain();
        if ( ba.isEmpty() )
            return {};

        qCInfo(logCategory) << "Encryption password loaded from system keychain";
    }
    Encryption::SecureArray sa(ba.value());
    return sa;
}
...
Encryption::SecureArray promptForNewPassword(QWidget *parent)
{
...
Encryption::EncryptionKey firstPasswordSetup(QWidget *parent)
{
    ...
    const Encryption::SecureArray storedPassword = getStoredPassword();
    if (!storedPassword.isEmpty())
        return setUpPassword(storedPassword);

    const Encryption::SecureArray newPassword = promptForNewPassword(parent);
...
Encryption::EncryptionKey promptForEncryptionPassword(QWidget *parent, PasswordPrompt prompt)
{
    ...
    if (prompt == PasswordPrompt::UseEnvAndKeychain) {
        const Encryption::SecureArray storedPassword = getStoredPassword();
        if ( !storedPassword.isEmpty() ) {
...
Encryption::EncryptionKey promptForEncryptionPasswordChange(QWidget *parent)
{
    ...
    const Encryption::EncryptionKey currentKey = promptForEncryptionPassword(parent, PasswordPrompt::IgnoreEnvAndKeychain);
...
Encryption::EncryptionKey promptForEncryptionPassword(QWidget *, PasswordPrompt)
{
    return {};
}
