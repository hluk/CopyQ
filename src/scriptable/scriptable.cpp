...
void installObject(QObject *fromObj, const QMetaObject *metaObject, QJSValue &toObject, QJSEngine *engine)
{
    static int i = 0;
    ...
    auto fnWrapper = engine->globalObject().property("_copyqFnWrapper");
    auto createProperty = engine->globalObject().property("_copyqCreateProperty");
    ...
    for (int i = 0; i < metaObject->methodCount(); ++i) {
        ...
        const auto fn = from.property(name);
        const auto v = fnWrapper.call(QJSValueList() << fn << hasByteArrayReturnType);
        if ( v.isError() ) {
            log( QString("Exception while wrapping %1.%2: %3").arg(fromName, name, v.toString()), LogError );
            Q_ASSERT(false);
        } else {
    ...
    for (int i = 0; i < metaObject->propertyCount(); ++i) {
        ...
        const auto v = createProperty.call(
            QJSValueList() << name << from << toObject << prop.isReadable() << prop.isWritable());
        if ( v.isError() ) {
            log( QString("Exception while adding property %1.%2: %3").arg(fromName, name, v.toString()), LogError );
            Q_ASSERT(false);
        }
    }
}
    ...
    , m_input()
{
    ...
    evaluateStrict(m_engine,
        ...
        "_copyqThrow = function(text) {"
        "throw new Error(text);"
        "}"
    );

    installObject(this, &Scriptable::staticMetaObject, globalObject, m_engine);
...
QJSValue Scriptable::throwError(const QString &errorMessage)
{
    ...
    auto fn = engine()->globalObject().property("_copyqThrow");
    const auto exc = fn.call(QJSValueList() << errorMessage);
...
QJSValue Scriptable::getPlugins()
{
    ...
    if ( m_plugins.isUndefined() ) {
        ...
        for (auto obj : scriptableObjects) {
            ...
            installObject(obj, obj->metaObject(), plugin, m_engine);
