...
{
    return self()->setTextModeEnabled(enabled);
}
...
void ScriptableFile::setFile(QFile *file)
{
    ...
    if ( !m_path.isNull() ) {
        const auto currentPath = engine()->globalObject().property("currentPath").toString();
        m_self->setFileName( QDir(currentPath).filePath(m_path) );
    }
}
...
{
    const auto byteArray = engine()->fromScriptValue<ScriptableByteArray*>(value);
